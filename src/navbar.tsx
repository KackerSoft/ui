...
export default function NavBar(props: NavBarProps) {
  ...
  return (
    <div
      className={cn(
        "fixed z-50 inset-x-6 flex items-center bottom-[calc(var(--safe-area-inset-bottom,1rem))] rounded-full border border-primary-100/15 bg-primary-950/50 backdrop-blur-2xl overflow-hidden shadow-lg opacity-100 transition-all",
        className,
      )}
    >
      {links.map((link) => (
        <Link
          ...
          href={link.href}
          className={cn(
            "w-full justify-center text-primary-50/60 text-center flex flex-1 items-center text-xl p-5 transition-all relative",
